...
## 8. stream
### 介绍
   stream和queue的功能类似，是对queue的一个补充，主要是为了应对4种新的应用场景
   > 1. `大规模广播`：在规模庞大的广播场景中，用户必须为每一个消费者绑定一个专用队列，当消费者数量庞大的时候性能会受到限制，尤其涉及到持久化或者复制的时候。stream使得所有消费者可以从同一个stream中消费同一个
   > 2. `重做`: 现存所有类型的queue都具有销毁属性，即被消费的message会被销毁，从而无法重读已经消费的message。而stream可以。
   > 3. `吞吐量`: 没有任何持久化队列类型能够提供与现有基于日志的消息系统相媲美的吞吐量。流（Streams）的设计将高性能作为一个重要目标。
   > 4. `message大量积压`: 大多数 RabbitMQ 队列被设计为趋向于空状态，并针对这一特性进行了优化，因此当某个队列中存在数百万条消息时，其性能可能会下降。相比之下，流（Streams）被设计为高效存储大量数据，并且内存开销最小。